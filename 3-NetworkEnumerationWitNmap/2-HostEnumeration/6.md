...
```shell-session
[!bash!]$ sudo nmap 10.129.2.0/24 -F
...
```shell-session
[!bash!]$ sudo nmap 10.129.2.0/24 -F --initial-rtt-timeout 50ms --max-rtt-timeout 100ms
...
```shell-session
[!bash!]$ sudo nmap 10.129.2.0/24 -F -oN tnet.T5 -T 5
...
```shell-session
[!bash!]$ cat tnet.T5 | grep "/tcp" | wc -l
